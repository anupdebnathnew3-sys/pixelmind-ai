...
import { Scale, Plus, Trash2, Check, X, Edit3, Save } from 'lucide-react';
...
export const TermsManagerPage: React.FC = () => {
  ...
  const saveMeta = () => {
    updateLegalContent({ termsTitle: meta.title, termsLastUpdated: meta.lastUpdated, termsVersion: meta.version });
    toast.success('Terms meta saved');
  };

  const addSection = () => {
    addLegalSection('terms', { title: `New Section ${legalContent.termsSections.length + 1}`, body: 'Section content goes here.' });
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">Edit your terms of service content</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Toggle checked={legalContent.termsEnabled} onChange={v => updateLegalContent({ termsEnabled: v })} label="Page visible" />
        </div>
      </div>
        ...
        <Card>
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-bold text-gray-900 dark:text-white">
              Sections <span className="ml-2 text-xs font-normal text-gray-400">{legalContent.termsSections.length} total</span>
            </h2>
          ...
          <div className="space-y-3">
            {legalContent.termsSections.map(section => (
              <SectionRow
                key={section.id}
                section={section}
                onUpdate={(id, patch) => updateLegalSection('terms', id, patch)}
                onDelete={(id) => removeLegalSection('terms', id)}
              />
            ))}
          </div>
        </Card>
